...
function useTechnologiesApi() {
  ...
  const searchTechnologies = async (techNames = ['react', 'node', 'python', 'javascript']) => {
    try {
      setLoading(true);
      setError(null);
      ...
      for (const techName of techNames) {
        try {
          const response = await fetch(`${TECH_API_URL}${techName}&sort=stars&per_page=1`);
          ...
          if (data.items && data.items.length > 0) {
            ...
            technologiesData.push({
              ...
              description: repo.description || `Информация о технологии ${techName}`,
  ...
  const addTechnology = async (techData) => {
    try {
      ...
      const newTech = {
        ...
        progress: techData.status === 'completed' ? 100 : 
                 techData.status === 'in-progress' ? 50 : 0
      };
        ...
        prev.map(tech => 
          tech.id === techId 
            ? { 
                ...tech, 
                ...updates,
                progress: updates.status === 'completed' ? 100 : 
                         updates.status === 'in-progress' ? 50 : 
                         updates.progress !== undefined ? updates.progress : tech.progress
              }
            : tech
        )
      );
